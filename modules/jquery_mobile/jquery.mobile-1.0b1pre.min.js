...
return a.Deferred().resolve(b,c,d,e).promise()};a.mobile.defaultTransitionHandler=a.mobile.noneTransitionHandler;a.mobile.transitionHandlers={none:a.mobile.defaultTransitionHandler};a.mobile.allowCrossDomainPages=!1;a.mobile.loadPage=function(b,d){var e=a.Deferred(),g=a.extend({},a.mobile.loadPage.defaults,d),i=b;fileUrl=m.getFilePath(i);dupCachedPage=page=null;g.pageContainer=g.pageContainer||a.mobile.pageContainer;if(g.data&&g.type==="get"){if(a.type(g.data)==="object")g.data=a.param(g.data);b+=
b+"?"+g.data;g.data=c}page=g.pageContainer.children(":jqmData(url='"+i+"')");t&&t.reset();if(page.length){if(!g.reloadPage)return f(page,g.role),e.resolve(b,d,page),e.promise();dupCachedPage=page}g.showLoadMsg&&a.mobile.showPageLoadingMsg();a.ajax({url:fileUrl,type:g.type,data:g.data,dataType:"html",success:function(c){var h=a("<div></div>"),j=c.match(/<title[^>]*>([^<]*)/)&&RegExp.$1,k=RegExp("\\bdata-"+a.mobile.ns+"url=[\"']?([^\"'>]*)[\"']?");RegExp(".*(<[^>]+\\bdata-"+a.mobile.ns+"role=[\"']?page[\"']?[^>]*>).*").test(c)&&
RegExp.$1&&k.test(RegExp.$1)&&RegExp.$1&&(b=fileUrl=m.getFilePath(RegExp.$1));t&&t.set(fileUrl);h.get(0).innerHTML=c;page=h.find(":jqmData(role='page'), :jqmData(role='dialog')").first();j&&!page.jqmData("title")&&page.jqmData("title",j);if(!a.support.dynamicBaseTag){var l=m.get(fileUrl);page.find("[src], link[href], a[rel='external'], :jqmData(ajax='false'), a[target]").each(function(){var b=a(this).is("[href]")?"href":"src",c=a(this).attr(b),c=c.replace(location.protocol+"//"+location.host+location.pathname,
"");/^(\w+:|#|\/)/.test(c)||a(this).attr(b,l+c)})}page.attr("data-"+a.mobile.ns+"url",fileUrl).appendTo(g.pageContainer);f(page,g.role);i.indexOf("&"+a.mobile.subPageUrlKey)>-1&&(page=g.pageContainer.children(":jqmData(url='"+i+"')"));g.showLoadMsg&&a.mobile.hidePageLoadingMsg();e.resolve(b,d,page,dupCachedPage)},error:function(){t&&t.set(m.get());g.showLoadMsg&&(a.mobile.hidePageLoadingMsg(),a("<div class='ui-loader ui-overlay-shadow ui-body-e ui-corner-all'><h1>"+a.mobile.pageLoadErrorMessage+"</h1></div>").css({display:"block",
opacity:0.96,top:h.scrollTop()+100}).appendTo(g.pageContainer).delay(800).fadeOut(400,function(){a(this).remove()}));e.reject(b,d)}});return e.promise()};a.mobile.loadPage.defaults={type:"get",data:"undefined",reloadPage:!1,role:"page",showLoadMsg:!0,pageContainer:c};a.mobile.changePage=function(g,h){if(p)n.unshift(arguments);else{p=!0;var k=a.extend({},a.mobile.changePage.defaults,h);k.pageContainer=k.pageContainer||a.mobile.pageContainer;if(typeof g=="string"){var l=a.mobile.loadPage(g,k);if(l){l.done(function(b,
c,d,e){p=!1;c.duplicateCachedPage=e;a.mobile.changePage(d,c)}).fail(function(){p=!1;d(!0);b()});return}}var o=k.pageContainer,l=a.mobile.activePage,t=g.jqmData("url");m.getFilePath(t);var v=i.getActive(),q=0,s=document.title,u=k.role==="dialog"||g.jqmData("role")==="dialog";o.trigger("beforechangepage");if(v&&v.page[0]===g[0])p=!1;else{f(g,k.role);k.fromHashChange&&i.directHashChange({currentUrl:t,isBack:function(){q=-1},isForward:function(){q=1}});a(document.activeElement||"").add("input:focus, textarea:focus, select:focus").blur();
u&&v&&(t=v.url+r);if(k.changeHash!==!1&&t)i.ignoreNextHashChange=!0,m.set(t);(u=g.jqmData("title")||g.children(":jqmData(role='header')").find(".ui-title").text())&&s==document.title&&(s=u);q||i.addNew(t,k.transition,s,g);document.title=i.getActive().title;a.mobile.activePage=g;k.transition=k.transition||(q?v.transition:c)||(k.role==="dialog"?a.mobile.defaultDialogTransition:a.mobile.defaultPageTransition);k.reverse=k.reverse||q<0;l=e(g,l,k.transition,k.reverse,k.changeHash);l.done(function(){d();
k.duplicateCachedPage&&k.duplicateCachedPage.remove();j.removeClass("ui-mobile-rendering");b();o.trigger("changepage")})}}};a.mobile.changePage.defaults={transition:c,reverse:!1,changeHash:!0,fromHashChange:!1,role:"page",duplicateCachedPage:c,pageContainer:c};a("form").live("submit",function(b){var c=a(this);if(a.mobile.ajaxEnabled&&!c.is(":jqmData(ajax='false')")){var d=c.attr("method"),e=m.clean(c.attr("action")),f=c.attr("target");!m.isExternal(e)&&!f&&(m.isRelative(e)&&(e=m.makeAbsolute(e)),
a.mobile.changePage(e.length&&e||m.get(),{type:d.length&&d.toLowerCase()||"get",data:c.serialize(),transition:c.jqmData("transition"),direction:c.jqmData("direction"),reloadPage:!0}),b.preventDefault())}});a(document).bind("vclick",function(b){if(b=g(b.target)){var c=m.clean(b.getAttribute("href")||"#");c!=="#"&&c.replace(m.get(),"")!=="#"&&a(b).closest(".ui-btn").not(".ui-disabled").addClass(a.mobile.activeBtnClass)}});a(document).bind("click",function(b){var c=g(b.target);if(c){var e=a(c),f=e.attr("href")||
"#",c=m.hasProtocol(f),f=m.clean(f),i=e.is("[rel='external']"),h=m.isEmbeddedPage(f),j=a.mobile.allowCrossDomainPages&&location.protocol==="file:"&&f.search(/^https?:/)!=-1,i=m.isExternal(f)&&!j||i&&!h,h=e.is("[target]"),j=e.is(":jqmData(ajax='false')");if(e.is(":jqmData(rel='back')"))return window.history.back(),!1;f.replace(m.get(),"")=="#"?b.preventDefault():(l=e.closest(".ui-btn"),i||j||h||!a.mobile.ajaxEnabled?window.setTimeout(function(){d(!0)},200):(i=e.jqmData("transition"),h=(h=e.jqmData("direction"))&&
h==="reverse"||e.jqmData("back"),e=e.attr("data-"+a.mobile.ns+"rel")||"page",m.isRelative(f)&&!c&&(f=m.makeAbsolute(f)),f=m.stripHash(f),a.mobile.changePage(f,{transition:i,reverse:h,role:e}),b.preventDefault()))}});h.bind("hashchange",function(){var b=m.stripHash(location.hash),d=a.mobile.urlHistory.stack.length===0?"none":c;if(!a.mobile.hashListeningEnabled||i.ignoreNextHashChange)i.ignoreNextHashChange=!1;else{if(i.stack.length>1&&b.indexOf(r)>-1)if(a.mobile.activePage.is(".ui-dialog")){var e=
function(){b=a.mobile.urlHistory.getActive().page};i.directHashChange({currentUrl:b,isBack:e,isForward:e})}else{i.directHashChange({currentUrl:b,isBack:function(){window.history.back()},isForward:function(){window.history.forward()}});return}b?a.mobile.changePage(b,{transition:d,changeHash:!1,fromHashChange:!0}):a.mobile.changePage(a.mobile.firstPage,{transition:d,changeHash:!1,fromHashChange:!0})}})})(jQuery);
...
j,k=null,m=!1,l=!0;a(function(){a(document).bind("vmousedown",function(){l&&(k=f)}).bind("vclick",function(b){l&&!a(b.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length&&!m&&(a.fixedToolbars.toggle(k),k=null)}).bind("scrollstart",function(){m=!0;k==null&&(k=f);var b=k=="overlay";if(g=b||!!h)a.fixedToolbars.clearShowTimer(),b&&a.fixedToolbars.hide(!0)}).bind("scrollstop",function(b){a(b.target).closest("a,input,textarea,select,button,label,.ui-header-fixed,.ui-footer-fixed").length||
(m=!1,g&&(g=!1,a.fixedToolbars.startShowTimer()),k=null)}).bind("silentscroll",d);a(window).bind("resize",d)});a(".ui-page").live("pagebeforeshow",function(b,c){var d=a(b.target).find(":jqmData(role='footer')"),f=d.data("id"),g=c.prevPage,g=g&&g.find(":jqmData(role='footer')"),g=g.length&&g.jqmData("id")===f;f&&g&&(j=d,e(j.removeClass("fade in out").appendTo(a.mobile.pageContainer)))});a(".ui-page").live("pageshow",function(){var b=a(this);j&&j.length&&setTimeout(function(){e(j.appendTo(b).addClass("fade"));
j=null},500);a.fixedToolbars.show(!0,this)});a(".ui-collapsible-contain").live("collapse expand",d);return{show:function(c,d){a.fixedToolbars.clearShowTimer();f="overlay";return(d?a(d):a.mobile.activePage?a.mobile.activePage:a(".ui-page-active")).children(".ui-header-fixed:first, .ui-footer-fixed:not(.ui-footer-duplicate):last").each(function(){var d=a(this),f=a(window).scrollTop(),g=b(d[0]),h=window.innerHeight,j=d.outerHeight(),f=d.is(".ui-header-fixed")&&f<=g+j||d.is(".ui-footer-fixed")&&g<=f+
h;d.addClass("ui-fixed-overlay").removeClass("ui-fixed-inline");!f&&!c&&d.animationComplete(function(){d.removeClass("in")}).addClass("in");e(d)})},hide:function(b){f="inline";return(a.mobile.activePage?a.mobile.activePage:a(".ui-page-active")).children(".ui-header-fixed:first, .ui-footer-fixed:not(.ui-footer-duplicate):last").each(function(){var c=a(this),d=c.css("top"),d=d=="auto"?0:parseFloat(d);c.addClass("ui-fixed-inline").removeClass("ui-fixed-overlay");if(d<0||c.is(".ui-header-fixed")&&d!=
0)b?c.css("top",0):c.css("top")!=="auto"&&parseFloat(c.css("top"))!==0&&c.animationComplete(function(){c.removeClass("out reverse");c.css("top",0)}).addClass("out reverse")})},startShowTimer:function(){a.fixedToolbars.clearShowTimer();var b=a.makeArray(arguments);h=setTimeout(function(){h=c;a.fixedToolbars.show.apply(null,b)},100)},clearShowTimer:function(){h&&clearTimeout(h);h=c},toggle:function(b){b&&(f=b);return f=="overlay"?a.fixedToolbars.hide():a.fixedToolbars.show()},setTouchToggleEnabled:function(a){l=
a}}}}()})(jQuery);
