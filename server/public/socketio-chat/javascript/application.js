...
//    var socket = io.connect('', {'resource':loginurl + 'chat'});
    var resource = "socket.io"; // "chat";
    var transports =  [ "xhr-polling", "jsonp-polling" ];
    var socket = io.connect('http://localhost:8080', { 'resource': resource, transports: transports } );
    ...
    function message(msg) {
        ...
        if (!logged) {
            ...
            addMessage(json.author, json.message, me ? 'blue' : 'black', date ? new Date(date) : new Date());
        }
    }
